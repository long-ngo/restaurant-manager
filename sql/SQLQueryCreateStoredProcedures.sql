...
CREATE PROC USP_Get_Account_By_UserName
@userName VARCHAR(100)
AS
BEGIN
...
CREATE PROC USP_Update_Account --sửa hàm này
@useUserName VARCHAR(100),
...
BEGIN
	...
	SELECT @userNameAccount = UserName, @displayNameAccount = DisplayName, @typeAccount = Type FROM dbo.Account WHERE UserName = @userName

	IF (@useUserName <> @nameAccount)
	BEGIN
		IF (@userNameAccount = '')
		...
		BEGIN
		    IF (@nameAccount = @userName)
				IF (@displayNameAccount <> @displayName OR @typeAccount <> @type)
					UPDATE dbo.Account SET DisplayName = @displayName, Type = @type WHERE UserName = @nameAccount
		END
	END
	ELSE
	BEGIN
		IF (@userName = @nameAccount AND @displayNameAccount <> @displayName AND @type <> 2)
			UPDATE dbo.Account SET DisplayName = @displayName WHERE UserName = @nameAccount
	END		
END
GO
...
CREATE PROC USP_Update_Account_Login
@userName VARCHAR(100),
@displayName NVARCHAR(100),
@passWord VARCHAR(100),
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			DECLARE @displayNameAccount NVARCHAR(100) = N''
			SELECT @displayNameAccount = DisplayName FROM dbo.Account WHERE UserName = @userName

		    IF (@displayName <> @displayNameAccount)
				UPDATE dbo.Account SET DisplayName = @displayName WHERE UserName = @userName
		END
		ELSE
		BEGIN
		    DECLARE @passwordAccount VARCHAR(100) = ''
			SELECT @passwordAccount = PassWord FROM dbo.Account WHERE UserName = @userName

			IF (@newPassWord <> @passwordAccount)
...
BEGIN
	...
	SELECT @count = COUNT(id) FROM dbo.Food WHERE idCategory = @id
	...
	BEGIN
		...
		WHILE ((SELECT COUNT(*) FROM #tempFood) > 0)
		BEGIN
			SELECT TOP 1 @idFood = id FROM #tempFood

			DELETE dbo.BillInfo WHERE idFood = @idFood

		    DELETE #tempFood WHERE id = @idFood
		END
